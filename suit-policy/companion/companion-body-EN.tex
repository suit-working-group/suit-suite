...
\textbf{Historical university principle} &
  \textbf{Digital translation (\loc{status-table-subtitle})} \\
\midrule
Free movement of persons &
  \loc{status-byod} Functional BYOD,
  \loc{status-eduroam} eduroam,
  \loc{status-mobility} seamless researcher mobility \\
\addlinespace
Free movement of ideas &
  \loc{status-internet} unfiltered Internet access,
  \loc{status-cloud-ai} major cloud-based AI services,
  \loc{status-sci-sources} worldwide scientific sources \\
\addlinespace
Teacher-researcher autonomy &
  \loc{status-local-admin} local admin privileges (root/admin),
  \loc{status-tool-choice} freedom of tool choice,
  \loc{status-environments} configurable environments \\
\addlinespace
Scientific openness (Open Science) &
  \loc{status-data-platforms} access to data platforms,
  \loc{status-code-repos} code repositories,
  \loc{status-open-access} open-access publications \\
\addlinespace
Realistic training environments &
  \loc{status-labs} configurable labs,
  \loc{status-cyber-ranges} cyber-ranges,
  \loc{status-api-access} API access for AI courses \\
\bottomrule
\end{tabularx}
...
academic computing centres including ETH~Zurich's CSCS~\cite{eu-supercomputers-2020}~--- involved unauthorized access rather than data exfiltration, and
...
This is what MIT, Stanford, Oxford, ETH~Zurich, Cambridge, CERN,
Michigan and EPFL do --- and, through their national research-and-education
...
\begin{itemize}
  ...
  \item \textbf{Independent Commission for Reasonable Adjustments
    ...
    on the proportionality of digital restrictions. \loc{icradp-committee-precedent-lead} Refusals must be justified in writing.
\end{itemize}
...
authority\loc{cite-ilr-essential} pursuant to the continuous-improvement
...
example by \loc{nis2-national-transposition}\loc{cite-nis2-transposition}.
...
2. Pilot &
  ...
  a pilot technical department, a pilot research centre, and
  \loc{pilot-entities} ---
...
rely on \loc{nren-name} and its international
...
research-data path --- deployed, neither at the institution nor at
...
international research backbone through \loc{nren-name}, an \gls{hpc} facility moving
petabyte-scale traffic, and \loc{nren-csirt}\loc{cite-nren-csirt} for incident
...
\emph{co-designed} with \loc{nren-name} and shared across
...
\begin{enumerate}
  \item \textbf{AI as a structural tool for research and teaching:}
    ...
    Under \loc{ai-regulation}'s risk-based framework --- the
  ...
  \item \textbf{The applicable regulatory framework demands it:} the
    ...
    here by the national transposition of \gls{nis2}\loc{cite-nis2-transposition}, the
    institution \loc{essential-entity-modality} by \loc{nis2-supervisory-authority}
    as an \textbf{essential entity}
    (holistic perimeter, proactive supervision, mandatory audits,
    annual maturity report via \loc{national-notification-platform}\loc{cite-notification-platform})\loc{cite-ilr-essential};
  ...
  \item \textbf{Reference universities providing comparable openness:}
    ETH~Zurich, EPFL, Oxford, Cambridge, MIT, Stanford and many others
...
{\footnotesize\textbf{Accompanying documents:} \emph{``\suitpolicytitle\ --- \suitpolicysubtitle''}. The report provides the full state of the art, an international set of
...
for the international university network anticipated in phase~2 of
this initiative.}
